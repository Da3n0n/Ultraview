...
async function writeProfiles(context: vscode.ExtensionContext, profiles: DokployProfile[]): Promise<void> {
...
async function setActiveProfileId(context: vscode.ExtensionContext, profileId: string | undefined): Promise<void> {
...
export async function configureDokployUrl(context?: vscode.ExtensionContext): Promise<string | undefined> {
  if (context) {
    ...
    if (active) {
      const nextProfiles = profiles.map(profile => profile.id === active.id
  ...
  const profile = await promptForProfile(currentUrl
    ? { id: '', name: labelFromUrl(currentUrl), url: currentUrl }
    : undefined);
...
export class DokployProvider implements vscode.WebviewViewProvider {
  ...
  private async getState(): Promise<{ url: string; profiles: DokployProfile[]; activeProfileId?: string }> {
  ...
  private async editProfile(profileId: string): Promise<void> {
    ...
    const nextProfiles = profiles.map(item => item.id === profileId
      ? { ...item, name: input.name, url: input.url }
      : item);
    await writeProfiles(this.context, nextProfiles);
